...
export const getStockDetails = async (context: Context, ticker: string) => {
  ...
  let tickerDetails: ActionTickerDetails = await (
    await context.effects.stockDetails.getTickerDetailsApi(ticker)
  ).data.results;
  tickerDetails = {
    name: tickerDetails.name,
    ticker: tickerDetails.ticker,
    branding: tickerDetails.branding
      ? {
          logo_url: `${tickerDetails.branding.logo_url}?apiKey=${appConfig.apiKey}`,
        }
      : undefined,
  ...
  let tickerDailyDetails: ActionTickerDailyDetails = await (
    await context.effects.stockDetails.getTickerDailyDetailsApi(ticker, prevDay)
  ).data;
  tickerDailyDetails = {
    close: tickerDailyDetails.close,
  ...
  const stockDetails: ActionStockDetails = {
    ...tickerDetails,
    ...tickerDailyDetails,
  };
...
export const _toggleFulFiledState = (
  context: Context,
  isFulFilled: boolean
) => {
