...
const getStocks = (nextUrl?: string) => {
  return axios.get(
    nextUrl
      ? `${nextUrl}&apiKey=${appConfig.apiKey}`
      : getStocksUrl.replace('{0}', appConfig.apiKey)
  );
};
const getTickerDetails = (ticker: string) => {
  return axios.get(
    getTickerDetailsUrl.replace('{0}', ticker).replace('{1}', appConfig.apiKey)
  );
};
const getTickerDailyDetails = (ticker: string, date: string) => {
  return axios
    .get(
      getTickerDailyDetailsUrl
        .replace('{0}', ticker)
        .replace('{1}', date)
        .replace('{2}', appConfig.apiKey)
    )
    .catch((err) => ({
